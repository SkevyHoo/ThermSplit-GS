...
@torch.no_grad()
def get_render_path(scene:Scene1, spin_angle=5.0, n_frames=50 , spin_for=2):
    # 生成一个围绕场景的螺旋相机路径（spiral camera path）。
    # 它通过计算一组相机的平均位置和方向，围绕该方向生成一组相机位置，并返回这些相机对象
    """
    ...
    # Camera Objects.
    cameras = scene.thermal_couple_model.get_middle_cams()
