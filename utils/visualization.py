...
from scene.motion import ThermalCoupleModel
import torch
import torchvision
...
from scene.thermal_tran_enhance import TTEModel


class Visualizer:
    
    def __init__(self, dataset:ModelParams ,opt:OptimizationParams, scene: Scene1, gaussians: GaussianModel,
                 TTE,
                 bg_color,
        ...
        self.USE_REF = dataset.USE_REF_CAM
        self.USE_TTE = dataset.USE_TTE
        ...
        if self.TTE is not None:
            print(f"Visualizer has loaded TTE model !")
        else:
            print(f"Visualizer has not loaded TTE model !")
        ...
        n = len(scene.thermal_couple_model)
    ...
    def _get_visualization_camera(self, scene:Scene1, gaussians:GaussianModel, vis_cam_idx=None, threshold = 0.5):
        """
    ...
    def render_gaussian_and_cams(self, iteration):
        ...
        t = np.linspace(0, 1, len(self.scene.thermal_couple_model))[:,None]
    ...
    def sample_subframe_cams(self, idx, num_subframes=None):
        t = self.scene.thermal_couple_model._sample_nu_from_alignment(idx)
        if num_subframes is not None:
            subfr_idx = torch.linspace(0,t.shape[0]-1, num_subframes, device=t.device).long()
            t = t[subfr_idx]
        traj = self.scene.thermal_couple_model.get_trajectory(idx, t)
    ...
    def visualize_alignment(self, iteration):
        ...
        for i in range(3):
            for j in range(3):
                ...
                nu = self.scene.thermal_couple_model._sample_nu_from_alignment(idx).detach().cpu().numpy()
                nu_pivot = torch.sigmoid(self.scene.thermal_couple_model._nu[idx].sort().values).detach().cpu().numpy()
    ...
    def traj_render(self, current_iter):
        ...
        cmm:ThermalCoupleModel = self.scene.thermal_couple_model

        for i in range(len(self.scene.thermal_couple_model)):
            
            blur_retrieve, render_pkg_ref = cmm.query(cam_idx=i, 
                                           subframe_indice="all", 
                                           post_process=self.scene.tone_mapping)
            blurred = blur_retrieve["blurred"]
            gt = blur_retrieve["gt"]
            if self.USE_REF:
                ref_render = render_pkg_ref['render']
            else:
                ref_render = None

            if self.TTE is not None:
                blurred = blurred + self.TTE(blurred)
                ref_render = ref_render + self.TTE(ref_render) if ref_render is not None else None


            subframe_renders = blur_retrieve["subframes"]
            ...
            if ref_render is not None:
                torchvision.utils.save_image(ref_render.clamp(0.0,1.0),
                                             os.path.join(render_path, f"{i:03d}_ref.png"))
    ...
    def save_video(self):
        for video_frame_path in [self.alignment_path, self.traj_vis_path]:
            ...
            video_name = os.path.basename(full_path).split('.')[0].strip()
