...
class Scene1:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], curve_model=True, load_path=None):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")) or os.path.exists(os.path.join(args.source_path, "poses_bounds.npy")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args)
             ...
             scene_info = sceneLoadTypeCallbacks["RGBTS-T"](args)
        ...
        if curve_model:
            self.thermal_couple_model = ThermalCoupleModel(cam_infos=scene_info.train_cameras, args=args)
            self.thermal_couple_model.link_gaussian(gaussians=gaussians)
        else:
            self.thermal_couple_modele = None
        self.cam_order = []
        ...
        # Tone Mapping Function.  gamma used
    ...
    def get_random_cam_idx(self):
        ...
        if len(self.cam_order) == 0:
            self.cam_order = list(range(len(self.thermal_couple_model)))
            random.shuffle(self.cam_order)
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], curve_model=True, load_path=None):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")) or os.path.exists(os.path.join(args.source_path, "poses_bounds.npy")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args)
             ...
             scene_info = sceneLoadTypeCallbacks["RGBTS-R"](args)
        elif os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](args.source_path, args.white_background, args.eval)
        else:
