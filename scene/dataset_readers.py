...
def readColmapSceneInfo(args):
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,depths_params=depths_params,
                                           images_folder=os.path.join(path, reading_dir),
                                           depths_folder=os.path.join(path, args.depths) if args.depths != "" else "")
...
def readRGBTS_TSceneInfo(args):
    ...
    storePly(ply_path, xyz, rgb)
    if args.random_init:
        ply_path = os.path.join(path, "colmap/sparse/0/points3D_random_init.ply")
        # if not os.path.exists(ply_path):
            # Since this data set has no colmap data, we start with random points
        num_pts = 100_000
        print(f"Generating random point cloud ({num_pts})...")
...
sceneLoadTypeCallbacks = {
    ...
    "RGBTS-R": readRGBTS_RSceneInfo,
    "RGBTS-T": readRGBTS_TSceneInfo,
