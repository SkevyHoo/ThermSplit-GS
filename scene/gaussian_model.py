...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    ...
                                                    # lr_delay_mult=training_args.position_lr_delay_mult,  lr_delay-steps:0
                                                    max_steps=training_args.iterations)
        

    def update_learning_rate(self, iteration, opt, alignment_lr=None, subframe_weight_lr=None):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            # add lr for curve optim  # 经过 curve_lr_half_iter 次调用后，学习率会减半
            ...
            elif subframe_weight_lr is not None and param_group['name'] == "weight_function":
                param_group['lr'] = subframe_weight_lr
    ...
    def load_ply(self, path):
        ...
        # differ from inria-3dgs which save opacity belongs to (-wq , +wq),we save it in (0,1).
    ...
    #     self.denom[update_filter] += denom_count
        #将2d像素点的xy梯度传递到再update_filter中的3dg点上
        #update_filter：布尔掩码（boolean mask），标记需要更新的点
        #记录每个点被观察到的次数（或其他加权统计量），用于后续计算平均梯度。
